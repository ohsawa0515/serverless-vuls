...
def lambda_handler(event, context):
    ...
    if not event.has_key('year'):
        year = int(init_year)
    elif int(event['year']) < 2002 or int(event['year']) > d.year:
        year = int(init_year)
    else:
        year = int(event['year'])
    ...
    # fetch
    check = commands.getoutput('./go-cve-dictionary fetchnvd -dbtype=mysql -dbpath="%s" -log-dir=/tmp/vuls -years %d' % (db_path, year))
    print(check)
